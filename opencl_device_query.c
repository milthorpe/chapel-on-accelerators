...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < number_of_platforms; i++) {
    ...
    clGetPlatformInfo(my_platforms[i],         //platform
                      ...
                      total_buffer_length,     //param_value_size
                      &my_platform_version,    //param_value
                      &length_of_buffer_used); //param_value_size_ret
    printf("\t\tversion:\t%*.*s\n", (int)length_of_buffer_used,
           (int)length_of_buffer_used, my_platform_version);
    ...
    for (size_t j = 0; j < number_of_devices; j++) {
      ...
      char my_device_il_version[total_buffer_length];
      length_of_buffer_used = 0;
      clGetDeviceInfo(my_devices[j],           //device
                      CL_DEVICE_IL_VERSION,    //param_name
                      total_buffer_length,     //param_value_size
                      &my_device_il_version,   //param_value
                      &length_of_buffer_used); //param_value_size_ret
      printf("\t\t\tIL version:\t\t%*.*s\n\n", (int)length_of_buffer_used,
             (int)length_of_buffer_used, my_device_il_version);
    }
    printf("\n");
    free(my_devices);
  }
